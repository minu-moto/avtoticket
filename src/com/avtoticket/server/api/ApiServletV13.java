...
import com.avtoticket.shared.models.core.Radio;
 ...
 */
public class ApiServletV13 extends ExtHttpServlet {

	private static final long serialVersionUID = -3804917311407491620L;

	private static Logger logger = LoggerFactory.getLogger(ApiServletV13.class);
	...
	private static List<Radio> getRadios() throws Exception {
		List<Radio> ret = DB.getModels(Radio.class, new Where(), Radio.NAME);
		ret.forEach(dt -> {
			dt.remove(Radio.CLASS_NAME);
		});
		return ret;
	}
	...
	private static List<Station> getLocations() throws Exception {
		List<Station> ret = DB.getModels(Station.class, Where.isNotNull(Station.LAT).andIsNotNull(Station.LNG), null, false, false);
		ret.forEach(s -> {
			s.set("locality", s.getNaspunkt());
			s.remove(Station.VALUE_FIELD);
			s.remove(Station.DISPLAY_FIELD);
			s.remove(Station.CLASS_NAME);
			s.remove(Station.NASPUNKT_ID);
			s.remove(Station.STAV_ID);
			s.remove(Station.KPAS_ID);
			s.remove(Station.HOST);
			s.remove(Station.IS_DEPARTURE_POINT);
			s.remove(Station.NASPUNKT);
		});
		return ret;
	}
	...
	private static List<Station> getDests(Long depId) throws Exception {
		...
		for (Map.Entry<Station, List<Station>> s : stations.entrySet())
			if (depId.equals(s.getKey().getId())) {
				List<Station> ret = s.getValue();
				ret.forEach(st -> {
	...
	private static List<Passage> getPassages(String sessionId, Long depId, Long destId, Date date) throws Exception {
		...
		List<Passage> psgs = TicketServiceImpl.getPassages(depId, date, destId, sessionId);
		if (psgs == null)
			return null;
		Long childhood;
		try {
		...
		List<Passage> ret = new ArrayList<>();
		for (Passage p : psgs) {
			Passage psg = new Passage();
			psg.fill(p);

			psg.set("is_transit", Objects.equals(psg.getTranzit(), 1L));
			psg.set("is_raft", psg.getRaftFlag());
			psg.set("total_seats", psg.getTotalSeats());
			psg.set("free_seats", psg.getFreeSeats());
			psg.set("bus_mark", psg.getBusMark());
			psg.set("bus_type", psg.getBusType());
			psg.set("childhood", childhood);
			psg.remove("elementName");
			psg.remove(Passage.RAFT_FLAG);
			psg.remove(Passage.BUS_TYPE);
			psg.remove(Passage.BUS_MARK);
			psg.remove(Passage.FREE_SEATS);
			psg.remove(Passage.TOTAL_SEATS);
			psg.remove(Passage.TRANZIT);
			psg.remove(Passage.SALE_SEAT_LIST_LONG);
			psg.remove(Passage.SALE_SEAT_LIST);
			psg.remove(Passage.CLASS_NAME);
			psg.remove(Passage.TARIFF);
			psg.remove(Passage.PREF_TARIFF);
			psg.remove(Passage.BAG_TARIFF);
			psg.remove(Passage.RAFT_TARIFF);
			psg.remove(Passage.RAFT_TARIFF_CH);
			psg.remove(Passage.OB_STRAH);
			psg.remove(Passage.KOM_SBOR);
			psg.remove(Passage.TBTARIF);
			psg.remove(Passage.PASSAGE_TYPE);
			psg.remove(Passage.DEP_ID);
			psg.remove(Passage.DEST_ID);
			psg.remove(Passage.DEP_NAME);
			psg.remove(Passage.DEST_NAME);
			ret.add(psg);
		}
		return ret;
	}
	...
	private static void cleanTicket(Ticket t) {
		...
		t.set("dep_id", t.getFromId());
		t.set("dest_id", t.getToId());
	...
	private static List<Ticket> getTickets(Long userId, String hash) throws Exception {
		...
		ret.forEach(ApiServletV13::cleanTicket);
		return ret;
	}

	private static String registerUser(HttpServletRequest req, String login, String pass, String lastname, String firstname, String middlename,
		...
		if ((pass == null) || pass.isEmpty())
			throw new ApiException(HttpServletResponse.SC_FORBIDDEN, "Некорректный запрос - не указан пароль пользователя");
		...
		StoredProcs.core.registerUser(login, pass, code, lastname, firstname, middlename, docId, docSeries, docNumber, docIssue, phone, birthdate, sex, nationId);
		UserUtil.sendRegConfirm(req, login, pass, code);

		return "Регистрация прошла успешно. В ближайшее время на указанный адрес будет отправлено письмо с дальнейшими инструкциями.";
	}

	private static String confirmUser(HttpServletRequest req, String login, String code) throws Exception {
		if ((login == null) || login.isEmpty())
			throw new ApiException(HttpServletResponse.SC_FORBIDDEN, "Некорректный запрос - не указан логин пользователя");
		if ((code == null) || code.isEmpty())
			throw new ApiException(HttpServletResponse.SC_FORBIDDEN, "Некорректный запрос - не указан код подтверждения");
		...
		if (userId != null) {
			if (userId >= 0) {
				...
				return auth(userId, req.getHeader(HttpHeaders.USER_AGENT), remoteAddr);
			} else
				throw new ApiException(HttpServletResponse.SC_GONE, "Код подтверждения устарел, попробуйте ещё раз");
		} else
	...
	private static String auth(String login, String password, String ua, String ip) throws Exception {
		...
		return auth(uid, ua, ip);
	}

	private static String auth(Long uid, String ua, String ip) throws Exception {
	...
	private static BaseModel getTickets(String token, TicketStatus status, Long offset, Long limit) throws Exception {
		...
		tkts.forEach(ApiServletV13::cleanTicket);
		ret.set("items", tkts);
		return ret;
	}
	...
	private static String sellTicket(HttpServletRequest req, String userAgent, String token, String email, String cacheKey, String successUrl, String failUrl, String usrs) throws Exception {
		...
		return TicketServiceImpl.buyTickets(req, userAgent, customer, users, psg, true, successUrl, failUrl);
	}
		...
		try {
			...
			case "orders":
				if (GET.equalsIgnoreCase(req.getMethod()))
					...
					String redirect = sellTicket(req, req.getHeader(HttpHeaders.USER_AGENT), req.getParameter("access_token"), req.getParameter("email"), req.getParameter("cache_key"),
			...
			case "user":
				if (GET.equalsIgnoreCase(req.getMethod()))
					...
					ret = registerUser(req, req.getParameter(User.LOGIN), req.getParameter("password"), req.getParameter(User.LASTNAME), req.getParameter(User.FIRSTNAME),
			...
			case "restore":
				if (GET.equalsIgnoreCase(req.getMethod())) {
					...
					if ((user != null) && CommonServerUtils.isValidEmail(email)) {
						UserUtil.restorePassword(req, req.getParameter("login"));
						ret = "Пароль выслан на указанный адрес электронной почты";
					} else
						throw new ApiException(HttpServletResponse.SC_NOT_FOUND, "Пользователь с указанным E-mail не найден.");
				} else
					throw new ApiException(HttpServletResponse.SC_METHOD_NOT_ALLOWED, "Метод недоступен для данного ресурса");
				break;
			case "confirm":
				if (POST.equalsIgnoreCase(req.getMethod()))
					ret = confirmUser(req, req.getParameter("login"), req.getParameter("code"));
				else
					throw new ApiException(HttpServletResponse.SC_METHOD_NOT_ALLOWED, "Метод недоступен для данного ресурса");
				break;
			case "tickets":
			...
			responseJson(resp, e.getErrorCode(), "\"" + e.getMessage() + "\"");
		} catch (Exception e) {
			logger.error("Внутренняя ошибка сервера", e);
			responseJson(resp, HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "\"Внутренняя ошибка сервера\"");
		}
	}
